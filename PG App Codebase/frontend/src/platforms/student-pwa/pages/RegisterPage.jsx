...
import { registerInitiate, registerVerify } from '@shared/api/auth'
...
export default function RegisterPage() {
  ...
  async function handleSendOTP(e) {
    ...
    try {
      await registerInitiate(email.trim())
      startCooldown()
      setStep(2)
    } catch (apiErr) {
  ...
  async function handleVerify(e) {
    ...
    try {
      ...
      authLogin(res.accessToken, res.data)
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-8 shadow-card">
            <div className="mb-6">
              ...
              <p className="text-sm text-[#434849] mt-1">
                {step === 1 ? 'Join Nest Stay to discover and review PGs' : `We sent a 6-digit code to ${email}`}
              </p>
            </div>
            ...
            {step === 1 ? (
              <form onSubmit={handleSendOTP} className="space-y-5" noValidate>
                <div>
                  ...
                  <input
                    ...
                    aria-describedby={emailError ? 'spwa-reg-email-err' : undefined}
                  />
                  {emailError && (
                    <p id="spwa-reg-email-err" className="mt-1.5 text-xs text-red-600">{emailError}</p>
                  )}
                </div>
              ...
              <form onSubmit={handleVerify} className="space-y-5" noValidate>
                {/* OTP */}
                <div>
                  <label className="block text-sm font-semibold text-[#1b1c1c] mb-2" htmlFor="spwa-reg-otp">
                    Verification code
                  </label>
                  <input
                    id="spwa-reg-otp"
                    ...
                    aria-describedby={fieldErrors.otp ? 'spwa-reg-otp-err' : undefined}
                  />
                  {fieldErrors.otp && (
                    <p id="spwa-reg-otp-err" className="mt-1.5 text-xs text-red-600">{fieldErrors.otp}</p>
                  )}
                ...
                {/* Confirm password */}
                <div>
                  <label className="block text-sm font-semibold text-[#1b1c1c] mb-2" htmlFor="spwa-reg-confirm">
                    Confirm password
                  </label>
                  <div className="relative">
                    <input
                      ...
                      aria-describedby={fieldErrors.confirm ? 'spwa-reg-confirm-err' : undefined}
                    />
                    <button
                      type="button"
                  ...
                  {fieldErrors.confirm && (
                    <p id="spwa-reg-confirm-err" className="mt-1.5 text-xs text-red-600">{fieldErrors.confirm}</p>
                  )}
                </div>
