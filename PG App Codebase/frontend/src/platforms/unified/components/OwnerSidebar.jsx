...
export default function OwnerSidebar({ onClose, pendingCount = 0, pgName = null }) {
            ...
            <div className="space-y-0.5">
              {group.items.map(item => (
                <NavLink
                  ...
                  style={({ isActive }) => ({
                    color: isActive ? '#ffffff' : '#bac9ce',
                  })}
                >
                  {({ isActive }) => (
                    <>
                      <span style={{ color: isActive ? '#ffffff' : '#ffdbd0', opacity: isActive ? 1 : 0.7 }}>
                        {item.icon}
                      </span>
                      <span className="flex-1">{item.label}</span>
                      {item.to === '/pgowner/admissions' && pendingCount > 0 && (
                        <span className="bg-[#ffdbd0] text-[#3a0b00] text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[1.25rem] text-center leading-none">
                          {pendingCount}
                        </span>
                      )}
                    </>
